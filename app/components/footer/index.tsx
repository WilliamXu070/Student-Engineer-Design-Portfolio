...
const Footer = () => {
  ...
  const panelHeight = isMobile ? 2.6 : 2.2;
  ...
  return (
    <group position={[0, -44, 18]} rotation={[-Math.PI / 2, 0, 0]} ref={groupRef}>
      ...
      <mesh position={[contentX + (isMobile ? 0.62 : 0.75), 1.04, -0.04]}>
      ...
      <Text
        ...
        color="#bdd1e3"
        anchorX="left"
        position={[contentX, 1.18, 0]}>
        {FOOTER_COPY.eyebrow.toUpperCase()}
      </Text>
      <Text
        font="/soria-font.ttf"
        fontSize={isMobile ? 0.23 : 0.32}
      ...
      <Text
        ...
        anchorX="left"
        anchorY="top"
        position={[contentX, isMobile ? 0.2 : 0.08, 0]}>
        {FOOTER_COPY.body}
      </Text>

      <group position={[contentX, isMobile ? -0.96 : -0.78, 0]}>
        { getLinks() }
      </group>
    </group>
  );
